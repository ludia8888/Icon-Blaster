...
class BranchService:
    ...
    def __init__(
        ...
        self.cache = SmartCacheManager(self.tdb)
        self.diff_engine = diff_engine
        self.conflict_resolver = conflict_resolver
        self.event_publisher = event_publisher
        self.db_name = os.getenv("TERMINUSDB_DB", "oms")
        self.merger = JsonMerger()  # 신형 병합 엔진 사용
    ...
    async def initialize(self):
        ...
        try:
            ...
            await self.cache.warm_cache_for_branch(
                self.db_name, "main", ["Branch", "ChangeProposal", "MergeCommit"]
            )
        ...
        if self.event_publisher:
            try:
                await self.event_publisher.publish_branch_merged(
                    ...
                    proposal_id=proposal_id,
                    author=user_id,
                    merge_commit=result.merged_commit_hash
                )
            except Exception as e:
    ...
    async def _perform_merge(
        ...
        """
        신형 병합 엔진(JsonMerger)을 사용하여 3-way merge 수행
        """
        source_branch = proposal.source_branch
        target_branch = proposal.target_branch

        # 1. 공통 조상(base) 커밋 찾기
        base_commit_hash = await self._find_common_ancestor(source_branch, target_branch)
        if not base_commit_hash:
            raise RuntimeError(f"Could not find common ancestor for {source_branch} and {target_branch}")

        # 2. 각 브랜치/커밋의 전체 스키마를 순수 dict로 가져오기
        base_docs = await self.tdb.get_all_documents_as_dict(db=self.db_name, commit_id=base_commit_hash)
        source_docs = await self.tdb.get_all_documents_as_dict(db=self.db_name, branch=source_branch)
        target_docs = await self.tdb.get_all_documents_as_dict(db=self.db_name, branch=target_branch)
        
        # 3. 신형 병합 엔진 호출
        merge_result = await self.merger.merge(
            base=base_docs,
            ours=target_docs,  # 'ours' is the target branch we are merging into
            theirs=source_docs, # 'theirs' is the source branch with new changes
            strategy=MiddlewareMergeStrategy.AUTO_RESOLVE # TODO: Map strategy
        )
        
        # 4. 결과 변환 및 DB 적용
        if merge_result.success and not merge_result.has_conflicts:
            # 성공 시, 병합된 문서를 타겟 브랜치에 적용
            commit_message = f"Merge proposal '{proposal.title}' from {source_branch} into {target_branch}"
            await self.tdb.replace_documents(
                documents=list(merge_result.merged_value.values()),
                db=self.db_name,
                branch=target_branch,
                author=user_id,
                message=commit_message
            )
            # TODO: Create MergeCommit object
            
        # 5. BranchService의 MergeResult 모델로 변환하여 반환
        return MergeResult(
            success=merge_result.success and not merge_result.has_conflicts,
            has_conflicts=merge_result.has_conflicts,
            conflicts=[c.path for c in merge_result.conflicts], # Simplified
            merged_commit_hash=await self._get_branch_head(target_branch)
        )

    async def _find_common_ancestor(self, branch1: str, branch2: str) -> Optional[str]:
        """두 브랜치의 공통 조상 커밋을 찾음 (단순 구현)"""
        # 실제 Git과 같은 복잡한 로직 대신, 여기서는 target 브랜치의 부모를 base로 가정
        branch2_info = await self._get_branch_info(branch2)
        if branch2_info and branch2_info.get('parent'):
             # This is a simplification. A real implementation would traverse commit history.
            parent_branch_head = await self._get_branch_head(branch2_info['parent'])
            return parent_branch_head
        
        # Fallback to main branch head
        return await self._get_branch_head("main")
