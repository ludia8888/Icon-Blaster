...
async def acquire_lock(
    ...
    if request.lock_type == LockType.MANUAL and not user.is_admin:
        raise HTTPException(
...
@router.get("/check-permission/{branch_name}")
async def check_write_permission(
    branch_name: str,
    resource_type: Optional[str] = Query(None),
    action: str = Query("write"),
    user: UserContext = Depends(get_current_user)
) -> Dict[str, Any]:
    """Check if write operations are allowed on a branch"""
    lock_manager = get_lock_manager()
