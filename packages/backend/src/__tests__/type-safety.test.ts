...
describe('Type Safety Requirements', () => {
  ...
  it('should support full generic types in asyncHandler', () => {
    type RequestHandler<TReq extends Request = Request, TRes extends Response = Response> = (
      req: TReq,
      res: TRes,
      next: NextFunction
    ) => Promise<void> | void;
  ...
  it('should support generic type transformation in middleware', () => {
    ...
    // 예: body 검증 미들웨어
    function validateBody<T extends z.ZodSchema>(schema: T): ValidationMiddleware<Request> {
      return (req, res, next) => {
